...
class Smartaddons_Slickslider_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	...
	protected function _toHtml(){
		...
		//$template_file = "smartaddons/slickslider/block_template_default.phtml";
		$template_file = 'smartaddons/slickslider/'. $this->_config["theme"].'.phtml';
		...
		foreach( $products as $k => $_product ) {
			...
			if($this->_config['product_image_disp']){
				if($_product->getImage()){
					...
					$item['thumb'] = $this->getSkinUrl('smartaddons/slickslider/images/no_image.png');
				}	
			}else{
				$item['thumb'] ='';
			}
	...
	public function getScriptTags(){
		...
		if (null == Mage::registry('jsmart.jquery')){
			...
			if (Mage::getStoreConfigFlag('slickslider_cfg/advanced/include_jquery')){
				...
				$import_str .= $jsHelper->includeSkinScript('smartaddons/slickslider/js/jquery-1.5.min.js');
				Mage::register('jsmart.jquery', 1);
			}
		}
		if (null == Mage::registry('jsmart.jquerynoconfict')){
			// add once noConflict
			$import_str .= $jsHelper->includeSkinScript('smartaddons/slickslider/js/jsmart.noconflict.js');
			Mage::register('jsmart.jquerynoconfict', 1);
		}
		
		if (null == Mage::registry('jsmart.slickslider.js')){
			// add script for this module.
			$import_str .= $jsHelper->includeSkinScript('smartaddons/slickslider/js/script.slickslider.js');
	...
	private function _getResizedImage($Obj, $width, $height, $quality = 100) {
		...
		if(file_exists($imageResized)){
			...
			return $this->getSkinUrl('smartaddons/slickslider/images/no_image.gif');
		}
	}		
}
